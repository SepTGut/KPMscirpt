...
<template>
  <div class="space-y-5">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
      <!-- KPI 1: Total Aktif -->
      <div class="rounded-2xl p-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm relative overflow-hidden transition hover:shadow-md">
        <div class="flex items-center justify-between">
          <span class="text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">Total KPM Aktif</span>
          <div class="w-8 h-8 rounded-xl bg-blue-50 dark:bg-blue-950/60 text-google-blue-600 dark:text-blue-400 flex items-center justify-center">
            <Icon name="doc" className="w-4 h-4" />
          </div>
        </div>
        <p class="text-2xl sm:text-3xl font-black text-slate-900 dark:text-white mt-2">
      ...
      <div class="rounded-2xl p-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm relative overflow-hidden transition hover:shadow-md">
        <div class="flex items-center justify-between">
          ...
          <div class="w-8 h-8 rounded-xl bg-amber-50 dark:bg-amber-950/60 text-amber-600 dark:text-amber-400 flex items-center justify-center relative">
            <span class="w-2 h-2 rounded-full bg-amber-500 absolute top-1 right-1 animate-ping"></span>
            <Icon name="truck" className="w-4 h-4" />
          </div>
        </div>
        <p class="text-2xl sm:text-3xl font-black text-amber-600 dark:text-amber-400 mt-2">
          {{ kpiStats.inTransit }}
        </p>
      ...
      <div class="rounded-2xl p-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm relative overflow-hidden transition hover:shadow-md">
        ...
        <p class="text-2xl sm:text-3xl font-black text-indigo-600 dark:text-indigo-400 mt-2">
    ...
    <div class="bg-white dark:bg-slate-900 p-3 sm:p-4 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm space-y-3">
      <div class="flex flex-wrap items-center justify-between gap-3">
        ...
        <div class="relative flex-1 min-w-[240px] max-w-md">
          ...
          <input
            ...
            class="w-full pl-9 pr-8 py-2 rounded-xl text-xs bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-google-blue-500 font-medium"
          />
          <button
            v-if="searchQuery"
            type="button"
        ...
        <div class="flex items-center gap-2 flex-wrap">
          ...
          <div class="inline-flex rounded-xl p-1 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700">
            <button
              ...
              :class="viewMode === 'grid' ? 'bg-white dark:bg-slate-700 text-slate-900 dark:text-white shadow-xs' : 'text-slate-500 dark:text-slate-400 hover:text-slate-900'"
            ...
            <button
              ...
              class="px-2.5 py-1 rounded-lg text-xs font-bold transition flex items-center gap-1"
              :class="viewMode === 'table' ? 'bg-white dark:bg-slate-700 text-slate-900 dark:text-white shadow-xs' : 'text-slate-500 dark:text-slate-400 hover:text-slate-900'"
          ...
          <div class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-xl bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 text-xs">
            <button
              type="button"
              @click="$emit('toggle-polling')"
              class="text-xs font-bold text-slate-600 dark:text-slate-300 hover:text-google-blue-600 dark:hover:text-google-blue-400 transition"
          ...
          <button
            v-if="isIT"
            type="button"
            class="rounded-xl bg-rose-50 hover:bg-rose-100 text-rose-700 dark:bg-rose-950/40 dark:text-rose-300 dark:border-rose-800 border border-rose-200 py-1.5 px-3 text-xs font-bold transition flex items-center gap-1.5 shadow-2xs focus-visible:outline-none"
            :disabled="busy"
      ...
      <div class="flex flex-wrap gap-1.5 pt-1 border-t border-slate-100 dark:border-slate-800">
        <button
          ...
          :class="filter === option ? 'bg-google-blue-600 text-white border-google-blue-600 shadow-sm' : 'bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border-slate-200 dark:border-slate-700 hover:bg-slate-200 dark:hover:bg-slate-700'"
    ...
    <div v-if="viewMode === 'grid' && displayedMonitoring.length" class="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
      <article
        ...
        :key="item.nomor"
        class="panel transition-all duration-200 flex flex-col justify-between"
      >
        <div>
          <div class="flex flex-wrap items-start justify-between gap-2 border-b border-slate-200 dark:border-slate-800 pb-3">
            <div>
              <div class="flex items-center gap-1.5 flex-wrap">
                <span class="text-xs font-mono font-bold text-google-blue-700 dark:text-blue-400 tracking-wider uppercase bg-blue-50 dark:bg-blue-950/60 px-2 py-0.5 rounded-md border border-blue-200 dark:border-blue-800">
                  {{ item.nomor }}
                </span>
                <span v-if="item.isTest" class="text-[10px] font-extrabold uppercase px-2 py-0.5 rounded-full bg-purple-100 text-purple-800 dark:bg-purple-950 dark:text-purple-300 border border-purple-300 dark:border-purple-800 flex items-center gap-1">
                  <span>🧪</span>
                  <span>TEST</span>
                </span>
              </div>
              <h3 class="text-base font-bold text-slate-900 dark:text-white mt-1.5">{{ item.proyek || 'Line Feeding' }}</h3>
            ...
            <div class="flex items-center gap-1.5">
              ...
              <div
                v-else
                class="text-[11px] font-bold py-1 px-2.5 rounded-full border inline-flex items-center gap-1.5 shadow-sm select-none"
                :class="statusClass(item.status)"
              >
          ...
          <div class="mt-3 grid gap-2 text-xs sm:grid-cols-2 text-slate-600 dark:text-slate-400 bg-slate-50 dark:bg-slate-800/60 p-3 rounded-2xl border border-slate-200 dark:border-slate-800">
            ...
            <p><span class="font-bold text-slate-800 dark:text-slate-200">Penerima:</span> <strong :class="item.penerima ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-400 font-normal'">{{ item.penerima || '-' }}</strong></p>
          </div>
          ...
          <div class="mt-3">
            ...
            <div class="h-2 overflow-hidden rounded-full bg-slate-200 dark:bg-slate-800">
              <div
                class="h-full rounded-full bg-gradient-to-r from-google-blue-500 to-emerald-500 transition-all duration-500"
          ...
          <div v-if="item.buktiBerangkat || item.buktiTiba || item.buktiDiterima" class="mt-2.5 flex flex-wrap gap-1.5">
            <a
              ...
              class="py-1 px-2.5 rounded-xl bg-blue-50 dark:bg-blue-950/60 hover:bg-blue-100 text-blue-700 dark:text-blue-300 text-[11px] font-bold inline-flex items-center gap-1 border border-blue-200 dark:border-blue-800 transition shadow-2xs"
            >
              <span>📷 Berangkat</span>
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              class="py-1 px-2.5 rounded-xl bg-amber-50 dark:bg-amber-950/60 hover:bg-amber-100 text-amber-800 dark:text-amber-300 text-[11px] font-bold inline-flex items-center gap-1 border border-amber-200 dark:border-amber-800 transition shadow-2xs"
            >
              <span>📷 Tiba</span>
              <Icon name="external" className="w-2.5 h-2.5" />
            </a>
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              class="py-1 px-2.5 rounded-xl bg-emerald-50 dark:bg-emerald-950/60 hover:bg-emerald-100 text-emerald-800 dark:text-emerald-300 text-[11px] font-bold inline-flex items-center gap-1 border border-emerald-300 dark:border-emerald-800 transition shadow-2xs"
            >
              <span>📸 Diterima</span>
              <Icon name="external" className="w-2.5 h-2.5" />
            </a>
          ...
          <details class="mt-3 rounded-2xl bg-slate-50 dark:bg-slate-800/50 p-2.5 border border-slate-200 dark:border-slate-800 text-xs">
            <summary class="cursor-pointer font-bold text-google-blue-700 dark:text-blue-400 outline-none flex items-center gap-1.5">
        ...
        <div class="mt-3 pt-3 border-t border-slate-100 dark:border-slate-800 flex flex-col gap-2">
          ...
          <button
            ...
            type="button"
            class="w-full py-2 px-3 rounded-xl bg-amber-50 dark:bg-amber-950/40 text-amber-800 dark:text-amber-300 border border-amber-300 dark:border-amber-800 text-xs font-bold flex items-center justify-center gap-1.5 transition hover:bg-amber-100"
            :disabled="busy"
    ...
    <div v-else-if="viewMode === 'table' && displayedMonitoring.length" class="panel !p-0 overflow-hidden border border-slate-200 dark:border-slate-800 shadow-sm">
      <div class="overflow-x-auto max-h-[70vh]">
        <table class="w-full text-left text-xs border-collapse">
          <thead class="sticky top-0 z-10 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200 border-b border-slate-200 dark:border-slate-700 text-[11px] font-black uppercase tracking-wider select-none">
            <tr>
              <th @click="setSort('nomor')" class="py-3 px-3 cursor-pointer hover:bg-slate-200 dark:hover:bg-slate-700">
                No KPM <span v-if="sortColumn === 'nomor'">{{ sortAsc ? '▲' : '▼' }}</span>
              </th>
              <th @click="setSort('proyek')" class="py-3 px-3 cursor-pointer hover:bg-slate-200 dark:hover:bg-slate-700">
              ...
              <th @click="setSort('status')" class="py-3 px-3 cursor-pointer hover:bg-slate-200 dark:hover:bg-slate-700">
          ...
          <tbody class="divide-y divide-slate-100 dark:divide-slate-800 bg-white dark:bg-slate-900">
            ...
            >
              ...
              <td class="py-2.5 px-3 whitespace-nowrap">
                <span :class="item.penerima ? 'text-emerald-700 dark:text-emerald-400 font-bold' : 'text-slate-400'">
                  {{ item.penerima || '-' }}
                </span>
              </td>
              <td class="py-2.5 px-3 whitespace-nowrap">
                <select
                  v-if="canOverrideStatus"
                  class="text-[10px] font-bold py-0.5 px-2 rounded-full border cursor-pointer outline-none"
                  :class="statusClass(item.status)"
                  :value="item.status"
                  :disabled="busy"
                  @change="$emit('change-status', item, $event)"
                ...
                <span
                  v-else
                  class="text-[10px] font-bold py-0.5 px-2 rounded-full border inline-block"
                  :class="statusClass(item.status)"
                >
                  {{ item.status }}
                </span>
              ...
              <td class="py-2.5 px-3 text-right whitespace-nowrap">
                <div class="flex items-center justify-end gap-1.5">
                  <button
                    v-if="item.status === 'Baru Dibuat' || item.status === 'Belum Berangkat'"
                    type="button"
                    class="p-1.5 rounded-lg text-amber-600 hover:bg-amber-50 dark:hover:bg-amber-950/40"
                    @click="$emit('edit-material', item)"
