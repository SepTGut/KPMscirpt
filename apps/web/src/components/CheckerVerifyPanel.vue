...
<template>
  <div class="w-full max-w-2xl mx-auto p-4 sm:p-6 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md rounded-3xl shadow-xl border border-slate-200/90 dark:border-slate-800 text-slate-800 dark:text-slate-100 animate-slideUpFade transition-colors">
    <!-- Header Card -->
    <div class="flex items-center gap-3 border-b border-slate-100 dark:border-slate-800 pb-4 mb-5">
      ...
      <button
        ...
        class="w-9 h-9 rounded-full bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 flex items-center justify-center text-sm font-bold transition-all duration-200 active:scale-90 flex-shrink-0 cursor-pointer"
    ...
    <div v-if="isVerified" class="text-center py-6 space-y-4 animate-springPop">
      ...
      <div class="pt-3">
        <button
          @click="emit('back-to-home')"
          class="btn-primary !px-6 !py-2.5 !text-xs font-bold shadow-md hover:shadow-lg transition-all cursor-pointer"
        >
    ...
    <div v-else class="space-y-4">
      ...
      <!-- Welcome Banner when no KPM selected -->
      <div v-if="!kpmId" class="p-3.5 bg-google-blue-50/80 dark:bg-blue-950/40 border border-google-blue-200 dark:border-blue-800/60 rounded-2xl text-xs text-google-blue-900 dark:text-blue-200 space-y-1">
        <div class="flex items-center gap-2 font-bold text-google-blue-800 dark:text-blue-300">
          <Icon name="shield" className="w-4 h-4 text-google-blue-600 flex-shrink-0" />
          <span>Pilih atau Masukkan Nomor KPM</span>
        </div>
        <p class="text-[11.5px] text-google-blue-700 dark:text-blue-300/90 leading-relaxed">
          Silakan pilih KPM dari daftar antrean di bawah atau ketik nomor fisik KPM untuk memeriksa muatan dan memverifikasi izin keluar gerbang (Gate Out).
        </p>
      </div>

      <!-- Driver Staging Status Banner when KPM selected but not staged -->
      <div v-else-if="!isStagedByDriver && !loadingDetail" class="p-3.5 bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800/60 rounded-2xl text-xs text-amber-900 dark:text-amber-200 space-y-1">
        ...
        <p class="text-[11px] text-amber-700 dark:text-amber-300/90 leading-relaxed">
          Driver untuk KPM <strong>{{ kpmId }}</strong> belum menekan tombol <strong>"Minta Izin Checker"</strong> di aplikasinya. Mohon minta driver menginisialisasi keberangkatan terlebih dahulu sebelum memeriksa.
        </p>
      </div>
